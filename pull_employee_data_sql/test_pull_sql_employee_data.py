...
from pull_sql_employee_data import (
    PullSqlEmployeeData,
    get_bool,
    get_date,
    parent_dir,
...
def query(table, column, con_param, s_date, e_date_none, exclude):
    if not e_date_none and con_param in ["=", "<", ">", "<=", ">="]:
        query_str = f"SELECT * FROM {table}  WHERE {column} {con_param}" f"'{s_date}'"
...
@pytest.fixture
def download_path():
    path = os.path.join(parent_dir, config["local"]["local_file_path"], "employee_sql")
    return path
...
class Test_sql_connection:
    ...
    @pytest.mark.xfail
    def test_read_query_is_not_done(
        self,
    ):
        """This method will test weather sql quey id read is done"""
        self.obj = SqlConnection(logger)
    ...
    def test_where_query_done(
        self, table, column, s_date, e_date_none, con_param, exclude
    ):
        """This test function will test whether where function successfully get data."""
        self.obj = SqlConnection(logger)
        response = self.obj.where_query(
            table, column, s_date, e_date_none, con_param, exclude
        )
        assert isinstance(response, pd.DataFrame)

    @pytest.mark.xfail
    def test_where_query_not_done(
        self, table, column, s_date, e_date, con_param, exclude
    ):
        """This test function will test whether where function successfully get data."""
        self.obj = SqlConnection(logger)
        response = self.obj.where_query(
            table, column, s_date, e_date, con_param, exclude
        )
        print(response)
        assert response is None
...
class Test_pull_sql_employee_data:
    ...
    def test_get_employee_data_where_done(
        self, s_date, e_date_none, con_param, exclude, data_frame
    ):
    ...
    def test_create_json_file_done(
        self, s_date, e_date_none, con_param, exclude, data_frame, join_date
    ):  # --tested
    ...
    def test_get_partition_done(
        self, join_date, s_date, e_date, con_param, exclude
    ):  # --tested
    ...
    def test_get_partition_path_not_done(
        ...
        try:
            ...
            partition_path = None
        response = self.obj.get_partition(
            "sfsafa"
        )  # join_date will be wrong format for failure case
        assert response == partition_path
