...
free_sound_api_section = config["ebird_api"]
...
class Test_ebird_api:
    ...
    def test_get_historic_obseravations_is_done(self, sound_id):
        """This method will test the similar sound
        method is successfully get data from endpoint"""
        self.ebird_api = EbirdApi(config)
        response = self.ebird_api.get_historic_observations(regieon_code,date)
        assert isinstance(response, pd.DataFrame)

    @pytest.mark.xfail
    def test_get_historic_obseravations_is_not_done(self,):
        """This method will test the similar sound
    ...
    def test_user_packs_is_done(self, username):
        ...
        method is successfully get data from endpoint"""
        self.ebird_api = FreeSoundApi(config)
        response = self.ebird_api.user_packs(username)
        assert isinstance(response, pd.DataFrame)
    ...
    def test_user_packs_is_not_done(self, wrong_username):
        ...
        method is not successfully get data from endpoint"""
        self.ebird_api = FreeSoundApi(config)
        response = self.ebird_api.user_packs(wrong_username)
        assert not response
