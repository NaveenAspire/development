...
@pytest.fixture
def endpoint(woeid,date_str):
    return f"https://www.metaweather.com/api/location/{woeid}/{date_str}/"
...
def last_run():
    str_date = '2022-05-03'
    last_date = datetime.strptime(str_date,"%Y-%m-%d")
    return last_date
...
@pytest.fixture
def api_res(woeid,api_search_date):
    response = "https://www.metaweather.com/api/location/{woeid}/{date}/".format(woeid=woeid, date=api_search_date)
    return response
...
class Test_pull_weather_information_from_metaweather_api:
    ...
    def test_get_weather_information_for_given_dates_start_and_end_done(self,s_date,e_date):
    ...
    def test_get_weather_information_for_given_dates_last_run_done(self,s_date_none,e_date_none,last_run):
    ...
    def test_get_weather_information_for_given_dates_previous_date_done(self,s_date_none,e_date_none,last_run_none):
    ...
    def test_get_weather_information_between_two_dates_done(self,s_date,e_date):
    ...
    @pytest.mark.xfail
    def test_get_weather_information_between_two_dates_done(self,s_date_none,e_date_none,last_run_none):
    ...
    def test_get_weather_information_done(self,s_date,e_date):
    ...
    @pytest.mark.xfail    
    def test_get_weather_information_not_done(self,s_date,e_date):
        """This method will test whether it get weather information is not sucessful"""
        self.obj = PullWeatherInformationFromMetaWeatherApi(s_date, e_date)
        response = self.obj.get_weather_information(s_date)
        assert response is None
        
    def test_get_given_date_response_done(self,s_date,e_date,api_res,city):
    ...
    @pytest.mark.xfail
    def test_get_given_date_response_not_done(self,s_date_none,e_date,api_res,city):
    ...
    def test_create_json_file_done(self,s_date,e_date,api_res,city):
    ...
    @pytest.mark.xfail
    def test_create_json_file_not_done(self,city,s_date,e_date,api_res):
        ...
        for i in range(24):
            t_str = "{date}T{hour}".format(date=s_date, hour=str(i).zfill(2))
            new_df = res_df[(res_df.created.str.contains(t_str))]
            if new_df.empty: # new_df will  be empty for failure case
                break
        response = self.obj.create_json_file(new_df,city,t_str)
        assert response is None
        
    def test_get_partition_path_done(self,city,s_date,e_date):
    ...
    @pytest.mark.xfail
    def test_get_partition_path_not_done(self,city):
        """This method will test whether it get paartition is not sucessful"""
        self.obj = PullWeatherInformationFromMetaWeatherApi(s_date, e_date)
        try:
            date_obj = datetime.strptime(t_str, "%Y-%m-%dT%H") #t_str will be wrong format for failure case
            partition_path = date_obj.strftime(
                f"pt_city={city}/pt_year=%Y/pt_month=%m/pt_day=%d/pt_hour=%H/")
        except Exception as err:
            partition_path = None
    ...
    def test_get_date_done(self,date_str):
    ...
    @pytest.mark.xfail        
    def test_get_date_not_done(self,date_str):
